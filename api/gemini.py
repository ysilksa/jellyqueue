...
def generate():
    ...
    try:
        ...
        scheduling_keywords = ["schedule", "meeting", "calendar", "availability"]
        if any(word in prompt.lower() for word in scheduling_keywords):
            # Format availability data
            times = []
            for slot in availability:
                for time in slot["timeslots"]:
                    times.append(f'{slot["date"]} from {time["start_time"]} to {time["end_time"]}')
            available_time = times[0] if times else "No availability found"

            # Simulate Google Meet link generation
            meet_link = f"https://meet.google.com/{uuid.uuid4().hex[:3]}-{uuid.uuid4().hex[3:7]}-{uuid.uuid4().hex[7:10]}"

            response = f"You are available on {available_time}. Here's your Google Meet link: {meet_link}"
